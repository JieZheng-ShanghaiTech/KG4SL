...
def train(args, data, string):
    ...
    for train_kf, eval_kf in kf.split(data[6]):
        if (cross_validation == 6):
            print(str(cross_validation)+' cross validation stop!')
            break
        else:
                ...
                for step in range(args.n_epochs):
                    ...
                    if (loss_mean < best_loss_flag):
                        ...
                        if (stopping_step >= early_stopping_flag):
                            print(
                                'Early stopping is trigger at step: %.4f  loss: %.4f  test_auc: %.4f  test_f1: %.4f   test_aupr: %.4f' % (
                                step, loss_mean, test_auc, test_f1, test_aupr))
                            break
